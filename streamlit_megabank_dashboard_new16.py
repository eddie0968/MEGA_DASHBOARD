...
def load_dynamic2_data(file_path):
    ...
    try:
        ...
        for sheet in sheet_names:
            ...
            # Rename first column and convert other columns to string to avoid type confusion
            new_columns = {col: str(col) for col in df.columns}
            new_columns[df.columns[0]] = '公司'
...
st.set_page_config(page_title="兆豐金控業務視覺化儀表板", page_icon="logos/兆豐金.jfif", layout="wide", initial_sidebar_state="collapsed")
SUBSIDIARY_GROUPS = {
    '銀行': [
...
def overview_charts(bank, indicators, sort_by, category_filter, peer_metric, sub_type):
    ...
    fig = px.bar(
        ...
        color_discrete_map={'公股':'#A88C64','民營':'#9E28AC', '兆豐金控': '#F4B000'} ##A88C64 公股OLDCOLOR
    )
...
def peer_comparison(bank, target, peers, indicators, category_filter, key_suffix=""):
    ...
    for company in plot_data['公司'].unique():
        if company not in color_map:
            color_map[company] = '#A88C64' # Default blue for other companies
    ...
    st.download_button(
        ...
        mime="text/html",
        key=f"dl_peer_dot_plot_{key_suffix}"
    )
...
def group_radar(bank, indicators, peer_metric):
    ...
    # 其他分組
    for col, color, symbol in [(f'公股 {peer_metric}', '#2BA84A', 'circle'), (f'民營 {peer_metric}', '#9E28AC', 'square')]:
        text_color = ''
        if '公股' in col:
            text_color = '#00008B'
        elif '民營' in col:
...
# "選擇子公司類型" menu has been removed. All indicators are now loaded by default.
selected_sub_types = list(filtered_subsidiary_groups.keys())

# Calculate all_inds based on all subsidiary types
all_inds = []
...
def render_tab_同業比較全覽():
    ...
    if not indicators_for_comparison:
        ...
        peer_comparison(data_df, target, peers, indicators_for_comparison, category_filter, key_suffix="tab1")
...
def render_tab_差異比較分析_雷達圖():
    st.subheader("市占率比較分析(雷達圖)")
    st.markdown("此頁面讀取 `Dynamic2.xlsx` 的資料，讓您選定一個時間點，並比較不同業務指標類別(Sheet)的表現。" )

    # 1. Load data and get available time points and metrics
    data_sheets, sheet_names = load_dynamic2_data("Dynamic2.xlsx")
    if not sheet_names:
        st.warning("警告：未找到 'Dynamic2.xlsx' 或該檔案中沒有任何工作表。" )
        return

    # 2. Extract unique time points from ALL sheets based on user requirements
    time_points = set()
    # Iterate through sheets, excluding any with "ROE"
    for sheet in sheet_names:
        if "ROE" not in sheet.upper():
            df = data_sheets[sheet]
            # Iterate through columns, applying filters
            for col in df.columns[1:]: # Skip first column ('公司')
                col_str = str(col)
                # Skip columns containing "金額" or named "2025/06"
                if "金額" in col_str or col_str == "2025/06":
                    continue
                
                # Extract the 4-digit year from the column name
                match = re.match(r'(\d{4})', col_str)
                if match:
                    time_points.add(match.group(1))

    if not time_points:
        st.warning("在 'Dynamic2.xlsx' 的所有工作表中，都找不到符合條件的時間點欄位 (例如 '2024...')。" )
        return

    # 3. UI for Time and Sheet (Metric) Selection
    sorted_time_points = sorted(list(time_points), reverse=True)
    selected_time = st.selectbox(
        "步驟1: 請選擇資料時間點",
        options=sorted_time_points,
        key="global_time_selector_v7"
    )

    metric_options = [s for s in sheet_names if "ROE" not in s.upper()]
    if not metric_options:
        st.warning("在 'Dynamic2.xlsx' 中未找到任何市占率相關的工作表。" )
        return
    
    default_metrics = metric_options[:3] if len(metric_options) >= 3 else metric_options
    selected_sheets = st.multiselect(
        "步驟2: 請選擇至少3個指標類別 (Sheet)",
        options=metric_options,
        default=default_metrics,
        key="global_sheet_selector_v7"
    )

    # 4. Data processing and plotting
    if not selected_time or not selected_sheets:
        st.info("請完成時間點與指標類別的選擇。" )
        return

    if len(selected_sheets) < 3:
        st.warning("雷達圖至少需要3個業務指標才能繪製，請選擇足夠的指標類別。" )
        return

    # Get a list of all companies from all relevant sheets to be comprehensive
    all_companies = set()
    for sheet in selected_sheets:
        if sheet in data_sheets:
            all_companies.update(data_sheets[sheet]['公司'].unique())
    
    plot_data = pd.DataFrame({'公司': list(all_companies)})

    # For each selected sheet, find the column corresponding to the selected time and merge it
    for sheet_name in selected_sheets:
        if sheet_name in data_sheets:
            df = data_sheets[sheet_name]
            # Find the specific column in this sheet that starts with the selected time
            target_cols = [col for col in df.columns if str(col).startswith(selected_time) and "市占率" in str(col)]
            
            if target_cols:
                # Assume the first match is the correct one
                target_col = target_cols[0]
                temp_df = df[['公司', target_col]].rename(columns={target_col: sheet_name})
                plot_data = pd.merge(plot_data, temp_df, on='公司', how='left')
            else:
                # If no column for that time is found in the sheet, add a column of zeros
                plot_data[sheet_name] = 0
        else:
            plot_data[sheet_name] = 0

    plot_data = plot_data.fillna(0)

    # Add '類別' column (public/private)
    company_category_df = data_df[['公司', '類別']].copy()
    df_with_category = pd.merge(plot_data, company_category_df, on='公司', how='left')
    
    public_keywords = ['臺灣', '合庫', '第一', '華南', '兆豐']
    def assign_category(row):
        if pd.notna(row['類別']): return row['類別']
        for keyword in public_keywords:
            if keyword in row['公司']: return '公股'
        return '民營'
    df_with_category['類別'] = df_with_category.apply(assign_category, axis=1)

    # Call the existing group_radar function for plotting
    st.markdown("---")
    st.subheader(f"{selected_time} 市占率雷達圖")
    group_radar(df_with_category, selected_sheets, peer_metric)
...
def render_tab_歷年走勢圖():
    ...
    try:
        ...
        if not roe_years:
            ...
            company_colors = {
                '兆豐金控': '#F4B000', '富邦金控': '#9E28AC', '國泰金控': '#E4572E',
                '中信金控': '#FF7C43', '玉山金控': '#FFA600', '元大金控': '#9A348E',
                '台新金控': '#6A057F', '新光金控': '#F08080', '開發金控': '#C70039',
                '永豐金控': '#900C3F', '臺灣金控': '#A88C64', '合庫金控': '#2BA84A',
            ...
            for _, row in initial_df.iterrows():
                if row['公司'] == '兆豐金控':
                    ...
                    initial_colors.append('#A88C64')
                else:
                    initial_colors.append('#9E28AC')
            ...
            for year in years:
                ...
                for _, row in frame_df.iterrows():
                    if row['公司'] == '兆豐金控':
                        ...
                        frame_colors.append('#A88C64')
                    else:
                        frame_colors.append('#9E28AC')
        ...
        if selected_sheet and dynamic2_data:
            ...
            if selected_companies:
                ...
                if 'ROE' in selected_sheet.upper(): # Check for ROE in sheet name (case-insensitive)
                    ...
                    value_vars = [col for col in df_plot.columns if "市占率" in str(col) and col not in id_vars]
                ...
                if selected_sheet == "銀行_ROE":
                    ...
                    for company in selected_companies:
                        if company == '兆豐銀行':
                            ...
                            color_map[company] = '#A88C64' # Public color
                            line_dash_map[company] = 'dash'
                        else:
                            color_map[company] = '#9E28AC' # Private color
                            line_dash_map[company] = 'dash'
                else:
                    for company in selected_companies:
                        if company in ['兆豐金控', '兆豐銀行']:
                            ...
                            try:
                                ...
                                if category == '民營':
                                    color_map[company] = '#9E28AC'
                                elif category == '公股':
                                    color_map[company] = '#A88C64'
                                else:
...
# !!!!!!!PERPLEXITY TAB6
def render_ranking_metrics_grid(data_sheets, sheet_names, chart_configs):
    """Renders a 2x6 grid of metric cards showing ranking changes."""
    st.subheader("重點指標排名速覽")

    def get_time_columns(df, title):
        """Gets and sorts time-based columns from a dataframe, prioritizing '2025' for ROE and ignoring the last ROE column."""
        cols = [c for c in df.columns if c != "公司" and pd.api.types.is_numeric_dtype(df[c])]
        
        if "ROE" in title:
            cols_2025 = sorted([c for c in cols if str(c).startswith('2025')], key=str, reverse=True)
            other_cols = sorted([c for c in cols if not str(c).startswith('2025')], key=str, reverse=True)
            
            sorted_cols = cols_2025 + other_cols
            return sorted_cols[:-1]
        
        share_cols = [c for c in cols if "市占率" in str(c)]
        return sorted(share_cols, key=str, reverse=True)

    def render_metric_card(chart_config):
        """Renders a single metric card using custom HTML markdown."""
        cfg = chart_config
        title = cfg['title']
        value_str = "--"
        delta_html_content = "<div style='font-size: 0.9em; color: grey; margin-top: 8px;'>資料待補</div>"

        if not cfg["placeholder"] and cfg["sheet_index"] is not None and cfg["sheet_index"] < len(sheet_names):
            sname = sheet_names[cfg["sheet_index"]]
            df = data_sheets.get(sname)
            
            if df is not None and not df.empty:
                target_company = "兆豐銀行" if "銀行" in cfg["title"] else "兆豐金控"
                
                if target_company in df['公司'].unique():
                    time_cols = get_time_columns(df, title)
                    
                    if time_cols:
                        latest_col = time_cols[0]
                        
                        # Correctly get the rank for the target company
                        ranks = df[latest_col].rank(ascending=False, method='min')
                        target_rank_series = ranks[df['公司'] == target_company]
                        
                        if not target_rank_series.empty:
                            latest_rank = target_rank_series.iloc[0]
                        else:
                            latest_rank = np.nan

                        if pd.notna(latest_rank):
                            value_str = f"第 {latest_rank:.0f} 名"
                        
                        if len(time_cols) >= 2:
                            prev_col = time_cols[1]
                            
                            prev_ranks = df[prev_col].rank(ascending=False, method='min')
                            prev_target_rank_series = prev_ranks[df['公司'] == target_company]

                            if not prev_target_rank_series.empty:
                                prev_rank = prev_target_rank_series.iloc[0]
                            else:
                                prev_rank = np.nan

                            if pd.notna(latest_rank) and pd.notna(prev_rank):
                                rank_diff = prev_rank - latest_rank
                                color = "yellow"
                                symbol = ""
                                delta_str = "持平"

                                if rank_diff > 0:  # Improvement (rank number decreased)
                                    color = "red"
                                    symbol = "▲"
                                    delta_str = f"{abs(rank_diff):.0f}"
                                elif rank_diff < 0:  # Decline (rank number increased)
                                    color = "green"
                                    symbol = "▼"
                                    delta_str = f"{abs(rank_diff):.0f}"

                                if symbol:
                                    delta_span = f'<span style="font-size: 1.2em; color: {color};">{symbol} {delta_str}</span>'
                                else:
                                    delta_span = f'<span style="font-size: 1.2em; color: {color};">{delta_str}</span>'

                                prev_span = f'<span style="font-size: 0.9em; color: grey; margin-left: 10px;">前期: {prev_rank:.0f} 名</span>'
                                delta_html_content = f'<div style="margin-top: 8px;">{delta_span}{prev_span}</div>'
                            else:
                                delta_html_content = "<div style='font-size: 0.9em; color: grey; margin-top: 8px;'>前期排名: --</div>"
                        else:
                            delta_html_content = "<div style='font-size: 0.9em; color: grey; margin-top: 8px;'>無前期資料</div>"
                else:
                    delta_html_content = f"<div style='font-size: 0.9em; color: grey; margin-top: 8px;'>無 {target_company} 資料</div>"
            else:
                delta_html_content = "<div style='font-size: 0.9em; color: grey; margin-top: 8px;'>無資料</div>"

        card_html = f'''
        <div style="padding: 10px; border: 1px solid #444; border-radius: 8px; height: 125px;">
            <div style="font-size: 1em; color: #DDDDDD; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="{title}">{title}</div>
            <div style="font-size: 2em; font-weight: bold; margin-top: 8px;">{value_str}</div>
            {delta_html_content}
        </div>
        '''
        st.markdown(card_html, unsafe_allow_html=True)

    # Create 2 rows of 6 columns each
    # First row
    cols_row1 = st.columns(6)
    for i in range(6):
        with cols_row1[i]:
            if i < len(chart_configs):
                render_metric_card(chart_configs[i])

    # Second row
    cols_row2 = st.columns(6)
    for i in range(6):
        with cols_row2[i]:
            if (i + 6) < len(chart_configs):
                render_metric_card(chart_configs[i + 6])
...
# ===== Tab6 複數歷年折線圖（6x2 共 12 張） =====
    ...
    # Render the ranking metrics grid at the top
    render_ranking_metrics_grid(data_sheets, sheet_names, chart_configs)
    ...
    def get_time_columns(df, title, mode): # Add title as an argument
        # mode: "roe" or "share"
        cols = [c for c in df.columns if c != "公司"]
        if mode == "roe":
            numeric_cols = [c for c in cols if pd.api.types.is_numeric_dtype(df[c])]
            return sorted(numeric_cols, key=lambda x: str(x))
        else: # mode == "share"
            share_cols = []
            for col_name in cols:
                # Temporarily bypass numeric check for debugging
                normalized_col_name = str(col_name).replace(" ", "") # Remove all spaces
                if "市占率" in normalized_col_name: # Removed pd.api.types.is_numeric_dtype check
                    share_cols.append(col_name)
            return sorted(share_cols, key=lambda x: str(x))
    ...
    def render_line_chart(df, title, time_cols, entity_type, key_suffix, defaults):
        ...
        df_sel = df[df["公司"].isin(selected)].copy()
        ...
        for comp in selected:
            if entity_type == "bank":
                if comp == "兆豐銀行":
                    color_map[comp] = "#F4B000"
                    line_dash_map[comp] = "solid"
                elif comp in public_banks:
                    color_map[comp] = "#A88C64"
                    line_dash_map[comp] = "dash"
                else:
                    color_map[comp] = "#9E28AC"
                    line_dash_map[comp] = "dash"
            else:  # fhc
                if comp == "兆豐金控":
                    color_map[comp] = "#F4B000"
            ...
            # Manually extract x and y for the current company
            company_row = df_sel[df_sel['公司'] == comp]
            if company_row.empty:
                continue

            x_values_raw = [str(c) for c in time_cols]
            x_values = []
            for val in x_values_raw:
                match = re.match(r'(\d{4}/\d{2}(?:/\d{2})?)', val) # Matches YYYY/MM or YYYY/MM/DD
                if match:
                    x_values.append(match.group(1))
                else:
                    x_values.append(val) # Fallback if no date pattern found
            y_values = company_row[time_cols].iloc[0].values
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="lines+markers",
                name=comp,
                line=dict(color=color_map.get(comp), dash=line_dash_map.get(comp), width=4 if comp in ["兆豐銀行", "兆豐金控"] else 2),
                marker=dict(size=8)
            ))

            # Add company name to the right end of each line
            if len(x_values) > 0:
                fig.add_annotation(
                    x=x_values[-1],
                    y=y_values[-1],
                    ...
                    xanchor="left",
                    xshift=10,
                    font=dict(color=color_map.get(comp), size=10)
                )
        ...
        for col_idx, col in enumerate([col_left, col_right]):
            ...
            with col:
                ...
                if cfg["title"].startswith("金控ROE"):
                    time_cols = get_time_columns(df, cfg["title"], "roe")[:-1]
                    entity_type = "fhc"
                    ...
                    time_cols = get_time_columns(df, cfg["title"], "share")
                    # 其餘圖以金控層級預設
                    entity_type = "fhc"
                    defaults = default_fhc
...
def render_tab_dynamic_overview():
    st.subheader("動態指標總覽")
    st.markdown("此頁面讀取 `Dynamic2.xlsx` 的資料，讓您選擇特定工作表與指標，進行跨公司比較。" )
    ...
    # 2. UI Controls
    selected_sheet = st.selectbox("步驟1: 請選擇資料表 (Sheet)", options=sheet_names, key="dynamic_overview_sheet_selector")

    if selected_sheet and data_sheets:
        df_raw = data_sheets[selected_sheet].copy()

        # 3. Determine which columns to offer based on sheet name
        if "ROE" in selected_sheet.upper():
            prompt = "步驟2: 此為ROE相關資料，請選擇要比較的時間點"
            indicator_cols = [col for col in df_raw.columns if col != '公司' and pd.api.types.is_numeric_dtype(df_raw[col])]
        else:
            prompt = "步驟2: 此為市占率相關資料，請選擇要比較的指標"
            indicator_cols = [col for col in df_raw.columns if "市占率" in str(col) and pd.api.types.is_numeric_dtype(df_raw[col])]

        if not indicator_cols:
            st.warning(f"在 '{selected_sheet}' 工作表中未找到可供比較的數值欄位（如：ROE 或含有'市占率'的欄位）。" )
            return

        selected_indicator = st.selectbox(prompt, options=indicator_cols, key="dynamic_overview_indicator_selector")

        if selected_indicator:
            # 4. Prepare base DataFrame for the selected indicator
            if selected_indicator not in df_raw.columns:
                st.error(f"所選指標 '{selected_indicator}' 不在資料中。" )
                return
            
            base_df = df_raw[['公司', selected_indicator]].copy().dropna(subset=[selected_indicator])

            # Add '類別' for coloring
            company_category_df = data_df[['公司', '類別']].copy()
            base_df = pd.merge(base_df, company_category_df, on='公司', how='left')
            ...
            def assign_category(row):
                if pd.notna(row['類別']):
                    return row['類別']
                for keyword in public_keywords:
                    if keyword in row['公司']:
                        return '公股'
                return '民營'
            base_df['類別'] = base_df.apply(assign_category, axis=1)

            # --- 1. Peer Comparison Plot ---
            st.markdown("---")
            st.subheader("兆豐與所選同業差異比較")

            target_company = '兆豐金控'
            if any(bank_name in selected_sheet for bank_name in ['銀行', 'Bank']):
                target_company = '兆豐銀行'

            if target_company not in base_df['公司'].unique():
                mega_candidates = [c for c in base_df['公司'].unique() if '兆豐' in c]
                if mega_candidates:
                    target_company = mega_candidates[0]
                    st.info(f"此資料表預設目標公司為 '{target_company}'")
                else:
                    target_company = None
                    st.warning(f"資料表中無兆豐相關公司，無法進行差異比較。" )
            
            if target_company:
                peer_candidates = [c for c in base_df['公司'].unique() if c != target_company]
                default_peers = ['國泰金控', '富邦金控', '中信金控', '玉山金控', '國泰世華銀行', '台北富邦銀行', '中國信託銀行', '玉山銀行']
                
                selected_peers = st.multiselect(
                    "選擇比較同業 (1-4家)", 
                    options=peer_candidates, 
                    default=[p for p in default_peers if p in peer_candidates][:4],
                    key='dynamic_peer_select'
                )
                
                # The original peer_comparison function has its own download button
                peer_comparison(
                    bank=base_df, 
            ...
            # --- 2. Overview Bar Chart (Sorted Desc) ---
            st.markdown("---")
            st.subheader("整體同業比較長條圖")

            chart_df = base_df.copy()
            if category_filter in ['公股', '民營']:
                chart_df = chart_df[chart_df['類別'] == category_filter]

            chart_df = chart_df.rename(columns={selected_indicator: '數值'})
            chart_df['plot_group'] = np.where(chart_df['公司'].str.contains('兆豐'), '兆豐', chart_df['類別'])
            
            # Add ranking column
            chart_df['rank'] = chart_df['數值'].rank(ascending=False, method='min').astype(int)

            # Let Plotly handle the sorting for correct display order
            sorted_company_order = chart_df.sort_values('數值', ascending=True)['公司'].tolist()
            ...
            fig_bar = px.bar(
                ...
                color='plot_group',
                orientation='h',
                title=f"<b>{selected_sheet}：{selected_indicator}</b><br><sup>單位：%</sup>",
                color_discrete_map={'公股': '#A88C64', '民營': '#9E28AC', '兆豐': '#F4B000'},
                category_orders={'公司': sorted_company_order}
            )

            peer_data = chart_df[~chart_df['公司'].str.contains('兆豐')]['數值']
            if not peer_data.empty:
                mean_val = peer_data.mean()
                median_val = peer_data.median()
                fig_bar.add_vline(x=mean_val, line_dash="dot", line_color="red", name="同業平均")
                fig_bar.add_vline(x=median_val, line_dash="dash", line_color="orange", name="同業中位數")
                fig_bar.add_annotation(x=mean_val, y=0.95, yref='paper', text=f"同業平均: {mean_val:.2f}", showarrow=False, yshift=10, xanchor="right", bgcolor="rgba(255,255,255,0.5)")
                fig_bar.add_annotation(x=median_val, y=0.90, yref='paper', text=f"同業中位數: {median_val:.2f}", showarrow=False, yshift=10, xanchor="right", bgcolor="rgba(255,255,255,0.5)")

            # Add rank annotation for Mega
            mega_rows = chart_df[chart_df['plot_group'] == '兆豐']
            for _, row in mega_rows.iterrows():
            ...
            fig_bar.update_layout(
                height=max(500, len(chart_df) * 35),
            ...
            html_bytes_bar = fig_bar.to_html(include_plotlyjs='cdn').encode('utf-8')
            st.download_button(
                "下載長條圖 (HTML)",
                data=html_bytes_bar,
                file_name=f"dynamic_bar_{selected_sheet}_{selected_indicator}.html",
                mime="text/html",
                key="dl_dynamic_bar"
            )
            ...
            def highlight_mega(row):
                return ['background-color: #F4B000' if '兆豐' in row['公司'] else '' for _ in row]

            display_table = chart_df.sort_values('數值', ascending=False).reset_index(drop=True)
            st.dataframe(display_table[['公司', '類別', '數值', 'rank']].rename(columns={'rank': '排名'}).style.apply(highlight_mega, axis=1).format({'數值': "{:.2f}%"}), use_container_width=True)

            # Download button for table
            csv = display_table.to_csv(index=False).encode('utf-8-sig')
            st.download_button(
                "下載數據表 (CSV)",
                data=csv,
                file_name=f"dynamic_data_{selected_sheet}_{selected_indicator}.csv",
                mime="text/csv",
                key="dl_dynamic_csv"
            )

def render_tab_racing_chart():
    st.subheader("動態競賽圖")
    st.markdown("此頁面使用 `Dynamic2.xlsx` 的資料，讓您選擇一項指標，並以動態排序的長條圖（Bar Chart Race）觀察各家公司隨時間的排名變化。" )
    ...
    # 2. UI Controls
    # Let user select which sheet (indicator) to display
    selected_sheet = st.selectbox("步驟1: 請選擇要生成競賽圖的指標 (Sheet)", options=sheet_names, key="racing_chart_sheet_selector")

    if selected_sheet and data_sheets:
        df_raw = data_sheets[selected_sheet].copy()

        # Identify time-based columns, excluding columns with "金額"
        time_cols = [
            col for col in df_raw.columns 
            if col != '公司' 
            and pd.api.types.is_numeric_dtype(df_raw[col]) 
            and "金額" not in str(col)
        ]
        
        # Try to parse dates for better sorting, fallback to string sort
        try:
            # Attempt to convert to datetime, coercing errors
            sorted_time_cols = sorted(time_cols, key=lambda t: pd.to_datetime(t, errors='coerce', format='%Y'))
        except (ValueError, TypeError):
            # Fallback to string sorting if conversion fails
            sorted_time_cols = sorted(time_cols, key=str)


        if not sorted_time_cols:
            st.warning(f"在 '{selected_sheet}' 工作表中未找到可供比較的數值/時間欄位。" )
            return

        # Melt the dataframe to long format
        df_melted = df_raw.melt(
            id_vars=['公司'],
            value_vars=sorted_time_cols,
            var_name='時間',
            value_name='數值'
        )
        df_melted = df_melted.dropna(subset=['數值'])

        # Add rank for each time period
        df_melted['rank'] = df_melted.groupby('時間')['數值'].rank(method='first', ascending=False).astype(int)
        
        # Merge with main data_df to get '類別'
        df_melted = pd.merge(df_melted, data_df[['公司', '類別']], on='公司', how='left')
        
        # Handle cases where company is not in data_df (e.g. banks vs. holdings)
        public_keywords = ['臺灣', '合庫', '第一', '華南', '兆豐']
        def assign_category(row):
            if pd.notna(row['類別']): return row['類別']
            for keyword in public_keywords:
                if keyword in row['公司']: return '公股'
            return '民營'
        df_melted['類別'] = df_melted.apply(assign_category, axis=1)


        # UI for animation speed and bar gap
        animation_speed = st.slider("調整動畫速度 (毫秒)", min_value=200, max_value=2000, value=500, step=100, key="racing_speed")
        bar_gap = st.slider('調整Bar條間距', 0.0, 0.5, 0.1, 0.05, key="racing_gap")

        # --- Create the figure with manual frames ---
        fig = go.Figure()
        time_periods = df_melted['時間'].unique()

        # --- Initial Frame ---
        initial_time = time_periods[0]
        initial_df = df_melted[df_melted['時間'] == initial_time].sort_values('rank', ascending=True)
        ...
        def get_bar_colors(df):
            ...
            for _, row in df.iterrows():
                if '兆豐' in row['公司']:
                    colors.append('#F4B000') # Mega color
                elif row['類別'] == '公股':
                    colors.append('#A88C64') # Public
                else:
                    colors.append('#9E28AC') # Private
            return colors
        ...
        fig.add_trace(go.Bar(
            ...
            insidetextanchor='middle',
            marker=dict(
                color=get_bar_colors(initial_df),
                opacity=[1.0 if '兆豐' in c else 0.7 for c in initial_df['公司']]
            ),
            textfont={'color':'white', 'size':16}
        ))
        ...
        for t in time_periods:
            frame_df = df_melted[df_melted['時間'] == t].sort_values('rank', ascending=True)
            ...
            annotations.append(go.layout.Annotation(
                ...
                x=0.95, y=0.1, xanchor='right', yanchor='bottom',
                font=dict(size=80, color="#4169E1", family="Arial, sans-serif")
            ))
            ...
            frames.append(go.Frame(
                ...
                data=[go.Bar(
                    ...
                    insidetextanchor='middle',
                    marker=dict(
                        color=get_bar_colors(frame_df),
                        opacity=[1.0 if '兆豐' in c else 0.7 for c in frame_df['公司']]
                    ),
                    textfont={'color':'white', 'size':16}
                )],
                layout=go.Layout(annotations=annotations)
        ...
        sliders = [dict(
            active=0,
            currentvalue={"prefix": "時間: ", "font": {"size": 25}},
            pad={"t": 50},
            steps=[dict(
                label=str(t),
                method="animate",
        ...
        fig.update_layout(
            title=f'<b>{selected_sheet} - 動態競賽圖</b>',
            height=max(600, len(initial_df) * 35),
            margin=dict(l=150, r=40, t=100, b=40),
            bargap=bar_gap,
            yaxis=dict(autorange="reversed", showticklabels=False),
            xaxis=dict(title=selected_sheet),
            updatemenus=[dict(
                type="buttons",
                showactive=False,
                y=0,
                ...
                buttons=[
                    dict(
                        ...
                        args=[None, dict(
                            frame=dict(duration=animation_speed, redraw=True),
                            fromcurrent=True,
                            transition=dict(duration=animation_speed / 2, easing="linear")
                        )]
                    ), 
                    dict(
                        label="Pause",
...
TAB_RENDERERS = {
    "戰情室": render_tab_戰情室,
    "公司競賽圖": render_tab_racing_chart,
    "差異比較分析(雷達圖)": render_tab_差異比較分析_雷達圖,
    "動態指標總覽": render_tab_dynamic_overview,
    "自選指標趨勢": render_tab_自選指標趨勢,
    "業務指標熱力圖": render_tab_業務指標熱力圖,
    # "歷年走勢圖": render_tab_歷年走勢圖, # Hidden
    # "同業比較全覽": render_tab_同業比較全覽, # Hidden
}
...
if "tab_order" not in st.session_state:
    st.session_state["tab_order"] = DEFAULT_TAB_ORDER

# 5) 提供「拖曳排序」UI（可放在主畫面頂端或側欄 expander 中）
with st.expander("拖曳調整分頁順序", expanded=True):
...
# 6) 使用 st.radio 和 session_state 建立有狀態的分頁
if "active_tab" not in st.session_state or st.session_state.active_tab not in st.session_state.tab_order:
...
st.radio(
    ...
    index=st.session_state["tab_order"].index(st.session_state.active_tab), # 根據session_state設定預設顯示
    label_visibility="collapsed"
)

# 渲染被選擇的 tab
if st.session_state.active_tab in TAB_RENDERERS:
