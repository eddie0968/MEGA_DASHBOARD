...
def render_tab_差異比較分析_雷達圖():
    ...
    # 2. Extract unique time points from ALL sheets
    time_prefixes = set()
    for sheet in sheet_names:
        if "ROE" not in sheet.upper():
            ...
            for col in df.columns[1:]:
                ...
                if match:
                    time_prefixes.add(match.group(1))
    
    if not time_prefixes:
        st.warning("在 'Dynamic2.xlsx' 的所有工作表中，都找不到可識別的時間點欄位 (例如 '2024/12...')。")
        return

    # 3. UI for Time and Sheet (Metric) Selection
    sorted_time_points = sorted(list(time_prefixes), reverse=True)
        ...
        for comp in selected:
            ...
            if entity_type == "bank":
                if comp == "兆豐銀行":
                    ...
                    color, dash, width = "#A88C64", "dash", 2
                else:
                    color, dash, width = "#9E28AC", "dash", 2
            else:  # fhc
                if comp == "兆豐金控":
                    color, dash, width = "#F4B000", "solid", 4
